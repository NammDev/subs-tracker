...
export function CurrencyComboBox({ user, onSelect }: CurrencyComboBoxProps) {
  ...
  const [selected, setSelected] = useState<Currency>(data[user?.currencyCode ?? 'VND'])
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button
          variant='outline'
          className='w-[78px] px-3 gap-2 text-ellipsis justify-start overflow-hidden'
        >
          {loading ? <Loader className='w-4 h-4' /> : <DownArrowIcon className='w-4 h-4' />}
          {selected ? <>{selected.code}</> : <>Select currency</>}
        </Button>
          ...
          {Object.values(data).map((datum: Currency) => (
            ...
            >
              <CheckIcon
                className={cn(
                  'mr-2 h-3.5 w-3.5',
                  datum.code === selected ? 'opacity-100' : 'opacity-0'
                )}
              />
